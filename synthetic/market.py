...
class OrderBook:
    ...
    def _match(self, taker_side: int, size: int, owner: str, limit: int | None) -> tuple[list[Fill], int]:
        ...
        while size > 0 and volume:
            best = min(volume) if taker_side == BUY else max(volume)
    ...
    def market(self, side: int, size: int, owner: str) -> list[Fill]:
        """Market order: walks the opposite side; any size the book cannot absorb is dropped."""
        if size <= 0:
            raise ValueError("size must be positive")
        fills, _ = self._match(side, size, owner, limit=None)
        return fills
